...
bool SpellingTrie::construct(const char* spelling_arr, size_t item_size,
  ...
  printf("SpellingTrie::construct(), \n\t spelling_arr:%s, \n\t item_size:%d \n\t item_num:%d", spelling_arr, item_size, item_num);
...
//  for(int i = 0; i< spelling_num_; i++) {
//    LOGD("\t spelling_buf_[%d]:%c\n", i, spelling_buf_[i]);
//  }
...
SpellingNode* SpellingTrie::construct_spellings_subset(
  ...
  printf("SpellingTrie::construct_spellings_subset(), \n\t spelling_buf_:%s,\n\t spelling_size_:%d, \n\t level:%d, \n\t char_for_node:%c",
       spelling_buf_, spelling_size_, level, char_for_node);
...
bool SpellingTrie::load_spl_trie(FILE *fp) {
  ...
  if (fread(spelling_buf_, sizeof(char) * spelling_size_,
            spelling_num_, fp) != spelling_num_)
    return false;
  printf("SpellingTrie::load_spl_trie() \n\t spelling_buf_:%s", spelling_buf_);
